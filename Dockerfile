...
RUN apt-get update && DEBIAN_FRONTEND="noninteractive" apt-get install -y curl build-essential libssl-dev pkg-config protobuf-compiler cpp-riscv64-linux-gnu gcc-riscv64-linux-gnu binutils-riscv64-linux-gnu bison flex bc libclang-dev wget
ARG ARCH=amd64
RUN curl -LO https://github.com/cartesi/machine-emulator/releases/download/v0.18.1/cartesi-machine-v0.18.1_$ARCH.deb
RUN apt-get update && DEBIAN_FRONTEND="noninteractive" apt-get install -y \
    ./cartesi-machine-v0.18.1_$ARCH.deb \
    && rm -rf /var/lib/apt/lists/* \
    && rm cartesi-machine-v0.18.1_$ARCH.deb
